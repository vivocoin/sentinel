...
def test_valid_vivo_address():
    from vivolib import is_valid_vivo_address

    main = valid_vivo_address()
    test = valid_vivo_address('testnet')

    assert is_valid_vivo_address(main) is True
    assert is_valid_vivo_address(main, 'mainnet') is True
    assert is_valid_vivo_address(main, 'testnet') is False

    assert is_valid_vivo_address(test) is False
    assert is_valid_vivo_address(test, 'mainnet') is False
    assert is_valid_vivo_address(test, 'testnet') is True


def test_invalid_vivo_address():
    from vivolib import is_valid_vivo_address

    main = invalid_vivo_address()
    test = invalid_vivo_address('testnet')

    assert is_valid_vivo_address(main) is False
    assert is_valid_vivo_address(main, 'mainnet') is False
    assert is_valid_vivo_address(main, 'testnet') is False

    assert is_valid_vivo_address(test) is False
    assert is_valid_vivo_address(test, 'mainnet') is False
    assert is_valid_vivo_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from vivolib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % vivolib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(vivolib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(vivolib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(vivolib.blocks_to_seconds(16616)) == 2612035
